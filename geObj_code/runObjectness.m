...
function [boxes allboxes] = runObjectness(img,numberSamples,params)
...
% params.distribution_windows = 10000;

if length(params.cues)==1    
    %single cues
    distributionBoxes = computeScores(img,params.cues{1},params);
    indexSamples = scoreSampling(distributionBoxes(:,end),numberSamples,1);%sample from the distribution of the scores    
    allboxes = distributionBoxes;
